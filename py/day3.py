...
def power_consumption(powers: list[str]) -> tuple[int,int]:
    ...
    gamma = ["" for _ in range(power_len)]
    epsilon = ["" for _ in range(power_len)]
    n = len(powers)
    h = n // 2

    ans = [0 for _ in range(power_len)]
    for power in powers:
        # Count 1s only
        for i,bit in enumerate(power):
           ans[i] += 1 if bit == "1" else 0 

    for i,bit in enumerate(ans):
        is_one_common = bit >= h
        gamma[i] = "1" if is_one_common else "0"
